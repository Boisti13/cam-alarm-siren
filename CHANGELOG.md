...
- `Standby Indicator` switch entity (default on) to disable the idle green standby indicator entirely. Only
  affects standby — an active alarm still shows full red blinking regardless of this setting. (Initially named
  `Status LED`, which collided with the existing `Status LED` light entity's name in the HA UI — renamed.)
...
- Standby LED state (default green, 15% brightness) shown whenever the alarm isn't active, applied at boot and
  ...
  - The capture guard also checks `standby_indicator_enabled`, since `id(led).turn_off()` (used when the
    Standby Indicator switch is off) doesn't clear the light's color channels — only its on/off state — so
    without this, ending an alarm while the switch is off would capture the alarm's red as the new "standby"
    color.
